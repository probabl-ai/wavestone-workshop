...
#
# Exploratory data analysis of the credit card fraud dataset, run
# before designing the baseline model.
#
# - **Raw data** is read-only — `RAW` points at `data/creditcard.csv`
#   (the Kaggle download). This file never cleans or modifies it.
# - **Outputs** go under `EDA_DIR` (`<project>/data/`):
#   `eda_creditcard.html`, summarized in `eda.md`.
...
RAW.shape  # noqa: B018 -- bare expression is the cell's digest output
